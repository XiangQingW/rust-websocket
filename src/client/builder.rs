...
use http::header::{
    CONNECTION, HOST, ORIGIN, SEC_WEBSOCKET_ACCEPT, SEC_WEBSOCKET_EXTENSIONS, SEC_WEBSOCKET_KEY,
...
use codec::http::{HeaderIndices, HeadersAsBytesIter, ResponseHead, MAX_HEADERS};
...
use rustls::{ClientConfig, ClientSession, ClientSessionMemoryCache};
#[cfg(feature = "async-rustls")]
use std::{
    cell::RefCell,
    sync::Arc,
...
#[cfg(feature = "async")]
pub fn get_connection_infos() -> (Option<Duration>, Option<Duration>, Option<Duration>) {
...
#[cfg(feature = "async")]
thread_local!(static CONNECTION_INFOS: RefCell<(Option<Duration>, Option<Duration>, Option<Duration>)> =
...
impl<'u> ClientBuilder<'u> {
        ...
        S: Into<String>,
    {
        ...
        self.headers
            .insert(SEC_WEBSOCKET_PROTOCOL, WebSocketProtocol(protocols).into());
        self
    }
        ...
        I: IntoIterator<Item = Extension>,
    {
        ...
        self.headers
            .insert(SEC_WEBSOCKET_EXTENSIONS, extensions.into());
        self
    }
    ...
    pub fn key(mut self, key: [u8; 16]) -> Self {
        self.headers
            .insert(SEC_WEBSOCKET_KEY, WebSocketKey(key.into()).into());
        self.key_set = true;
        self
    }
    ...
    pub fn async_connect_secure(
        ...
        let tls_config = match tls_config {
            ...
            None => {
                ...
                tls_config.set_persistence(client_cache);
                Arc::new(tls_config)
            }
        };
        let host = match self.extract_host_ssl_conn_async() {
                Ok(h) => ::webpki::DNSName::from(h),
                Err(e) => return Box::new(future::err(e)),
            };
        let connector = async_imports::TlsConnector::from(tls_config);
    ...
    fn build_request(&mut self) -> String {
        ...
        self.headers.insert(
            CONNECTION,
            Connection(vec![ConnectionOption::ConnectionHeader(Ascii::new(
                "Upgrade".to_string(),
            ))]).into(),
        );
        ...
        if !self.version_set {
            self.headers
                .insert(SEC_WEBSOCKET_VERSION, WebSocketVersion::WebSocket13.into());
        }

        if !self.key_set {
            self.headers
                .insert(SEC_WEBSOCKET_KEY, WebSocketKey::new().into());
        }
    ...
    fn validate(&self, response: &ResponseHead) -> WebSocketResult<()> {
        ...
        if response
            .headers
            .get(UPGRADE)
            .and_then(|v| v.to_str().ok().map(|v| v.to_owned().to_lowercase()))
        ...
        if self.headers.get(CONNECTION) != Some(
            &(Connection(vec![ConnectionOption::ConnectionHeader(Ascii::new(
                "Upgrade".to_string(),
            ))]).into()),
        ) {
    ...
    #[cfg(any(feature = "async-rustls"))]
    fn extract_host_ssl_conn_async(
        &self
    ) -> WebSocketResult<::webpki::DNSNameRef> {
        let host = match self.url.host_str() {
            Some(h) => match ::webpki::DNSNameRef::try_from_ascii_str(h) {
                Ok(h) => h,
                Err(_) => {
                    return Err(WebSocketError::WebSocketUrlError(
                        WSUrlErrorKind::InvalidDnsName,
                    ))
                }
            },
            None => {
...
mod tests {
    ...
    fn build_client_with_protocols() {
        ...
        let protos: WebSocketProtocol = builder
            .headers
